...
export function ConfigTournamentRules() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await updateTournamentConfig(tournamentId!, { config_torneo: getConfigPayload() });
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (status === 409) {
          ...
          if (data?.campo) setErrors({ [data.campo]: data.error });
  ...
  const handlePublish = async () => {
    if (!configGuardada) { setGeneralError("Guarda la configuración antes de publicar."); return; }
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setGeneralError(err.response?.data?.detail ?? "No se pudo publicar el torneo.");
      } else {
        setGeneralError("Error de conexión.");
      }
    } finally {
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 p-6">
              ...
              <button type="button" onClick={handlePublish}
                ...
                {isPublishing ? (
                  ...
                  <><Rocket className="mr-2 h-4 w-4" />Publicar Torneo</>
                )}
              </button>
            </div>
          </form>
