...
import { Bot, LogIn, AlertCircle, Lock } from "lucide-react";
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 400) {
          ...
          setGeneralError(data?.detail ?? "Credenciales inválidas");
        } else if (status === 423) {
          // Escenario: cuenta bloqueada → extraer los minutos restantes del mensaje
          const message: string = data?.detail ?? "Cuenta bloqueada. Intente más tarde.";
