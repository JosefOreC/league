...
export async function loginUser(credentials: LoginRequest): Promise<LoginResponse> {
  const response = await api.post<LoginResponse>("/auth/login/", credentials, { withCredentials: false });
  return response.data;
}

// Función para registrar un nuevo usuario
export async function registerUser(userData: any): Promise<any> {
  const response = await api.post("/auth/register/", userData, { withCredentials: false });
  return response.data;
}
...
// Función para obtener el perfil completo del usuario autenticado
export async function getUserProfile(): Promise<User> {
  const response = await api.get<User>("/auth/me/", { withCredentials: false });
  return response.data;
}
