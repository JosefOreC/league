...
import { Activity, PlusCircle, Trophy, Medal, Star, AlertCircle, Save, CheckCircle, RefreshCw } from "lucide-react";
...
export function Results() {
  ...
  // Datos provenientes de la URL
  const { id: torneoId } = useParams<{ id: string }>();
  const [partidoId] = useState("p1"); // En una implementación real, esto vendría de la selección en la UI
    ...
    try {
      const resultados = [
        ...mockCriterios.map(c => ({
          ...
          registrado_por: "juez_principal" // En producción vendría del AuthContext
        })),
        ...mockCriterios.map(c => ({
          criterio_id: c.id,
          equipo_id: equipo2Id,
